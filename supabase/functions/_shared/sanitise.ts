...
//
// Mirrors `api/lib/sanitise.ts` exactly — these are pure string functions
// with no Node/Deno-specific APIs, so the two copies can be kept in lockstep
// by hand. (We avoid importing the Node module here so the Edge Function
// doesn't need a TypeScript build step.)
//
// Server-side defence-in-depth: the LLM's in-prompt rules are a second
// layer, but these checks run before any text reaches the model.
...
function stripControlChars(text: string): string {
    // Keep \n (0x0A) and \t (0x09); strip the rest of the C0 range plus DEL.
    ...
    for (const pat of INJECTION_PATTERNS) {
        if (pat.test(text)) {
            return {
                ...
                blocked: true,
                reason:
                    "Your message was flagged by our safety filters. Please rephrase your question.",
            };
        }
    }
