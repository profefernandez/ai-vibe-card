...
<!-- Phase 6b–f additions -->
- **Phase 6b — call-site refactor.** Every `db.query()` call site (60 across 15 files) migrated to its correct lane. Authed routes that read/write tenant data — `routes/tables.ts`, `routes/upload.ts`, `routes/functions/{query-content,scrape-site,verify-domain,test-api-connection}.ts`, the membership lookup in `middleware/requireRole.ts` — switched to `req.withClient!((c) => c.query(...))` so they run inside an RLS-aware transaction. Identity-establishing paths (`routes/auth.ts` login/register/logout, the session lookup in `middleware/auth.ts`), the legacy cross-org connections router later retired in favor of Edge Functions, the public visitor-chat handler (`lemonade-chat.ts`), anonymous feedback (`feedback.ts`), the audit-log writer (`lib/audit.ts`), the cron handlers (`refresh-sites.ts`, `prune-logs.ts`), and the `/robots.txt` + `/api/ready` endpoints in `index.ts` switched to `serviceDb`. The shutdown handler now awaits both pools. Pre-policy-enable this is purely an architectural reshuffle — behavior is identical because RLS is still off.
- **Phase 6c — policies created (RLS still disabled).** Migration `1700000007000_rls_policies` creates 14 policies, one or more per table:
  - Org-scoped tables (`profiles`, `sites`, `ai_preferences`, `api_connections`): `USING (organization_id = app_current_org_id())`.
  - Inherited via site (`site_pages`, `content_blocks`): `USING (site_id IN (SELECT id FROM sites))` — relies on the parent table's policy filtering recursively.
  - Cross-org by design (`connections`): `USING (requester_id = app_current_user_id() OR owner_id = app_current_user_id())`.
  - Identity (`users`, `memberships`, `organizations`, `sessions`): self-only reads.
  - Append-only (`audit_log`, `ai_feedback`): permissive WITH CHECK so anonymous code paths can write.
  - Service-only (`feedback_consumed`): no user-facing access; only `aivibe_service` reads/writes via `BYPASSRLS`.
  Two SQL helper functions `app_current_user_id()` / `app_current_org_id()` cast the GUC strings to UUID with safe NULL-on-unset semantics — a query made WITHOUT the `SET LOCAL` plumbing returns 0 rows rather than 500ing. Tables remain `DISABLE ROW LEVEL SECURITY` here; this migration is a no-op at runtime and exists purely so policy review can happen separately from enablement.
- **Phase 6d–f — staged enable.** Migrations `1700000008000`–`11000` turn on RLS in four batches: `ai_preferences` first (lowest blast radius), then `api_connections`, then the bulk of tenant data (`profiles`, `sites`, `site_pages`, `content_blocks`, `connections`), then identity + audit (`users`, `memberships`, `organizations`, `sessions`, `audit_log`, `ai_feedback`, `feedback_consumed`). Each migration ships as its own deployable unit so the operator can apply, soak, observe, then ramp — the original plan called for 24h staging soak between batches and these migrations support that without code changes.
- **End-to-end RLS isolation test** at `api/scripts/test-rls-isolation.ts`. Spins up two organizations + users in a real Postgres, runs every migration up, creates a non-superuser `rls_test_user` so policies actually gate (vs. the test-env superuser implicit BYPASSRLS), forces RLS on the table-owner path too, and asserts: cross-org isolation works (user A sees their org's `ai_preferences`/`profiles`/`memberships` only), queries made WITHOUT `SET LOCAL` return zero rows (safe failure), and bind-mismatched access is filtered. **7/7 cases pass against a real Postgres 16.**

### Operator runbook for the RLS rollout
1. Deploy through migration `1700000007000` (policies created, RLS still disabled). Verify nothing changes for users.
2. Out-of-band: `ALTER ROLE aivibe_service WITH LOGIN PASSWORD '...'`, then set `DATABASE_URL_SERVICE` in the production env. Without this, `serviceDb` falls back to `DATABASE_URL` and won't bypass RLS once enabled.
3. Run `1700000008000` (Stage A: `ai_preferences`). Soak 24h. Watch for empty admin AI-training tabs.
4. Run `1700000009000` (Stage B: `api_connections`). Soak 24h.
5. Run `1700000010000` (Stage C: `profiles`, `sites`, `site_pages`, `content_blocks`, `connections`). Soak 24h.
6. Run `1700000011000` (Stage D: identity + audit). Soak 24h.
7. Optional Stage E (`ALTER TABLE … FORCE ROW LEVEL SECURITY`) to subject the table owner too — defers to the operator since FORCE complicates future migrations (they'd need to run as a BYPASSRLS role).
...
- Removed `connections` from the generic `/api/tables` allowlist. The router treated `owner_id` as the sole ownership column, which left requesters unable to act on their own outgoing requests via this surface and created an asymmetric IDOR window. Connection lifecycle stayed on its dedicated application surface, which preserved requester and owner symmetry correctly.
...
- Migration `1700000005000_connections_unordered_unique` replaces the ordered `uq_connections_pair UNIQUE (requester_id, owner_id)` constraint with a functional unique index on `(LEAST(requester_id, owner_id), GREATEST(requester_id, owner_id))`. Without this, both `(A→B)` and `(B→A)` rows could coexist — the legacy application layer checked for the reciprocal pair in code but the DB had no opinion. The migration refuses to apply if any reciprocal duplicates exist (raises with the count) rather than auto-resolving — there's no universally correct merge.
